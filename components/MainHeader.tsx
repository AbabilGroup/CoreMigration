        ...
        <div className="relative group flex items-center h-full cursor-pointer">
            ...
            <div className="absolute top-[calc(100%-8px)] left-0 z-50 w-72 bg-white shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                <div className="p-6 flex flex-col">
                    ...
                    <h3 className="text-brand-secondary text-[18px] font-bold mb-6">{t(label)}</h3>
                ...
                <div className="flex flex-col bg-[#F8F5F1]">
                    {items.map(item => (
                        <Link key={item.label} href={item.href} onClick={onClose} className="flex items-center gap-3 px-8 py-4 border-b border-[#5A4125]/15 group">
                            ...
                            <span className="text-[#5A4125] text-xs font-bold group-hover:text-brand-dark-border transition-colors">{t(item.label)}</span>
                        </Link>
                    ))}
                </div>
            )}
        ...
        <header className="fixed inset-x-0 top-0 z-40 flex h-16 md:h-18 items-center justify-between px-4 md:px-10 text-[#5A4125] bg-[#F2EDE7] transition-all duration-300">
            ...
            <div className="hidden md:flex items-center space-x-6 h-full">
                ...
                <nav className="flex items-center space-x-6">
                    ...
                    {SHOW_FULL_NAV && (
                        <>
                            <HeaderDropdown label="Our Solutions" href="/our-solutions" items={[
                                { label: 'Direct & Specialist Recruitment', href: '/our-solutions/direct-specialist-recruitment' },
                                { label: 'Bulk Mobilization', href: '/our-solutions/bulk-mobilization' },
                                { label: 'Overseas Processing', href: '/our-solutions/overseas-processing' }
                            ]} />
                            <HeaderDropdown label="Industries" href="/industries" items={[
                                { label: 'Heavy Construction & Infrastructure', href: '/industries#heavy-construction-infrastructure' },
                                ...
                                { label: 'Hospitality & Catering', href: '/industries#hospitality-catering' },
                                { label: 'Facility Management & Logistics', href: '/industries#facility-management-logistics' }
                            ]} />
                            <HeaderDropdown label="Regional Coverage" href="/regional-coverage" items={[
                                { label: 'Middle East (GCC)', href: '/regional-coverage/middle-east' },
                                { label: 'United Kingdom', href: '/regional-coverage/united-kingdom' },
                                { label: 'Europe', href: '/regional-coverage/europe' }
                            ]} />
                        </>
                    )}
                    <HeaderDropdown label="Vacancies" href="/vacancies" items={[
                        { label: 'Browse Roles', href: '/vacancies' },
                        { label: 'Register as a Candidate', href: '/vacancies/register' }
                    ]} />
                    <Link href="/how-we-deliver" className="font-bold text-sm tracking-tight text-[#5A4125] hover:text-brand-dark-border">
                        {t("How We Deliver")}
                    </Link>
                ...
                {/* Language Switcher */}
                <div className="relative group/lang flex items-center h-full cursor-pointer border-l border-[#5A4125]/20 pl-4 py-4">
                    <button className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-[#5A4125]/20 hover:border-brand-dark-border hover:bg-[#5A4125]/5 text-xs font-bold transition-all text-[#5A4125] uppercase">
                    ...
                    {/* Dropdown Menu */}
                    <div className="absolute top-[calc(100%-8px)] right-0 z-50 w-44 bg-white shadow-xl opacity-0 invisible group-hover/lang:opacity-100 group-hover/lang:visible transition-all duration-300 rounded-lg overflow-hidden border border-gray-100">
                        <div className="py-1">
                            {LANGUAGES.map((lang) => (
                                <button
                                    key={lang.code}
                                    onClick={() => setLocale(lang.code)}
                                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left hover:bg-gray-50 transition-colors ${locale === lang.code ? 'font-bold text-[#5A4125] bg-gray-50' : 'text-gray-600'}`}
                                >
                    ...
                    <div className="flex flex-col w-full pb-10">
                        ...
                        {SHOW_FULL_NAV && (
                            <>
                                ...
                                />
                                <MobileHeaderDropdown label="Industries" onClose={() => setIsMobileMenuOpen(false)} items={[
                                    { label: 'Heavy Construction & Infrastructure', href: '/industries#heavy-construction-infrastructure' },
                                    { label: 'Healthcare & Life Sciences', href: '/industries#healthcare-life-sciences' },
                                    { label: 'Oil, Gas & Energy', href: '/industries#oil-gas-energy' },
                                    { label: 'Hospitality & Catering', href: '/industries#hospitality-catering' },
                                ...
                                <MobileHeaderDropdown label="Regional Coverage" onClose={() => setIsMobileMenuOpen(false)} items={[
                                    { label: 'Middle East (GCC)', href: '/regional-coverage/middle-east' },
                                    { label: 'United Kingdom', href: '/regional-coverage/united-kingdom' },
                                    { label: 'Europe', href: '/regional-coverage/europe' }
                                ]} />
                        ...
                        <MobileHeaderDropdown label="Vacancies" onClose={() => setIsMobileMenuOpen(false)} items={[
                            { label: 'Browse Roles', href: '/vacancies' },
                            { label: 'Register as a Candidate', href: '/vacancies/register' }
                        ]} />
                        <Link href="/how-we-deliver" className="px-6 py-4 text-[16px] font-bold text-[#5A4125] border-b border-[#5A4125]/20 w-full text-left hover:bg-[#5A4125]/5 hover:text-brand-dark-border transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                            {t("How We Deliver")}
                        </Link>
                        ...
                        <div className="px-6 py-6 border-b border-[#5A4125]/20 bg-[#5A4125]/5">
                            <p className="text-xs font-bold text-[#5A4125]/60 uppercase tracking-wider mb-3">
                                {t("Select Language")}
                            </p>
                                ...
                                {LANGUAGES.map((lang) => (
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border text-sm font-semibold transition-all ${
                                            locale === lang.code
                                                ? 'bg-[#5A4125] text-[#F2EDE7] border-[#5A4125]'
                                                : 'bg-white text-[#5A4125] border-[#5A4125]/10 hover:bg-[#5A4125]/5'
                                        }`}
                                    >
                                        <span className="text-base">{lang.emoji}</span>
                                        <span>{lang.name}</span>
